 ...
 */
fun Bundle.getCharOrNull(key: String): Char? = if (containsKey(key)) getChar(key) else null

/**
 * Safely retrieves a Boolean value from the Bundle.
 * Returns null if the key is not present, avoiding the default false value.
 */
fun Bundle.getBooleanOrNull(key: String): Boolean? = if (containsKey(key)) getBoolean(key) else null
 ...
 */
fun Bundle.getStringOrNull(key: String): String? = if (containsKey(key)) getString(key) else null
