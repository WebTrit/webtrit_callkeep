...
)
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): PIOSOptions {
      ...
      val iconTemplateImageAssetName = pigeonVar_list[2] as String?
      val maximumCallGroups = pigeonVar_list[3] as Long
      val maximumCallsPerCallGroup = pigeonVar_list[4] as Long
      val supportsHandleTypeGeneric = pigeonVar_list[5] as Boolean?
      val supportsHandleTypePhoneNumber = pigeonVar_list[6] as Boolean?
      val supportsHandleTypeEmailAddress = pigeonVar_list[7] as Boolean?
      val supportsVideo = pigeonVar_list[8] as Boolean
      val includesCallsInRecents = pigeonVar_list[9] as Boolean
      val driveIdleTimerDisabled = pigeonVar_list[10] as Boolean
...
)
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): PAndroidOptions {
      ...
      val incomingPath = pigeonVar_list[1] as String
      val rootPath = pigeonVar_list[2] as String
...
)
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): PCallkeepServiceStatus {
      ...
      val autoRestartOnTerminate = pigeonVar_list[1] as Boolean
...
interface PHostIsolateApi {
  fun setUp(callbackDispatcher: Long?, onStartHandler: Long?, onChangedLifecycleHandler: Long?, autoRestartOnTerminate: Boolean, autoStartOnBoot: Boolean, androidNotificationName: String?, androidNotificationDescription: String?, callback: (Result<Unit>) -> Unit)
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            ...
            val autoRestartOnTerminateArg = args[3] as Boolean
            val autoStartOnBootArg = args[4] as Boolean
            val androidNotificationNameArg = args[5] as String?
            val androidNotificationDescriptionArg = args[6] as String?
            api.setUp(callbackDispatcherArg, onStartHandlerArg, onChangedLifecycleHandlerArg, autoRestartOnTerminateArg, autoStartOnBootArg, androidNotificationNameArg, androidNotificationDescriptionArg) { result: Result<Unit> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(wrapError(error))
              } else {
...
class PDelegateBackgroundRegisterFlutterApi(private val binaryMessenger: BinaryMessenger, private val messageChannelSuffix: String = "") {
  ...
  fun onWakeUpBackgroundHandler(userCallbackHandleArg: Long, statusArg: PCallkeepServiceStatus, dataArg: String, callback: (Result<Unit>) -> Unit)
{
    val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
    ...
    channel.send(listOf(userCallbackHandleArg, statusArg, dataArg)) {
      if (it is List<*>) {
        if (it.size > 1) {
    ...
    channel.send(listOf(callIdArg, handleArg, displayNameOrContactIdentifierArg, videoArg)) {
      if (it is List<*>) {
        if (it.size > 1) {
          ...
          callback(Result.failure(HostCallsDartPigeonFlutterError("null-error", "Flutter api returned null value for non-null return value.", "")))
        } else {
