...
> **Detailed docs** are in `docs/`. Key references:
> - `docs/architecture-overview.md` — component map and design decisions
> - `docs/call-flows.md` — end-to-end call flow diagrams
> - `docs/processes-and-ipc.md` — IPC events and `ConnectionPerform` table
> - `docs/services.md` — per-service lifecycle and state machines
...
**Inter-process communication** between these processes uses local broadcasts (`ConnectionServicePerformBroadcaster`) with `ConnectionPerform` events as actions. Direction is always `:callkeep_core` → main process. See `docs/processes-and-ipc.md` for the full event table.

**Self-managed `PhoneAccount`:** The plugin registers with `CAPABILITY_SELF_MANAGED`. The app does not need `CALL_PHONE` permission for its own VoIP calls; the system shows its own call UI (lock screen, status bar). `PhoneConnectionService` runs in `:callkeep_core` so Telecom state persists even when the main process is backgrounded.
...
- `CallMetadata` — primary data carrier passed through intents/bundles between processes; contains `callId`, `handle`, `displayName`, `hasVideo`, `ringtonePath`, audio state, etc. Serialized to/from `Bundle` (not `Parcelable`) to avoid cross-process class-loading issues.
...
- `MainProcessConnectionTracker` — companion object on `ForegroundService`; mirrors active connections in the main process via `ConnectionAdded`/`ConnectionRemoved` IPC events
