...
Registered once during `ForegroundService.setUp()` via `NotificationChannelManager.registerNotificationChannels()`.
...
**Full-screen intent:** Points to the app's launch activity so the call UI opens over the lock screen.
...
**Delete action (user swipes away):** The notification sets a delete intent with action `ACTION_RESTORE_NOTIFICATION` back to `SignalingIsolateService`. When the user dismisses the notification, `onStartCommand` detects this and calls `ensureNotification()` to re-post it, keeping the service alive.
...
The action strings are prefixed with `ContextHolder.appUniqueKey` (the app's package name) to avoid conflicts with other apps on the device.
