...
| PR-7a-fix | `fix/android-foreground-service-start-call-race` | `not started` | `invokeAndRemove` returns `Boolean`, guard `performStartCall` behind it. Standalone bugfix, no dependencies. |
...
baseline and apply only the delta.
...
| After out-of-plan #163–#165 rewrites: which exact delta does PR-6 still need to add? | PR-6a/6b/6c | **open** — run `git diff origin/develop..feat/android-callkeep-core-process-migration -- '*.kt'` for ConnectionManager + ForegroundService before starting |
...
- PR-6 + PR-7b-coldstart: develop baseline has changed significantly from the feature
  branch version of those files. Always diff before extracting.
